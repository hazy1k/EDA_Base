...
void Show_Chinese(u16 x,u16 y,u16 index,u8 size,u16 pointcolor,u16 backcolor,u8 mode)
{
	...
	{   												   
		...
		if(size==12)temp=hz12[index][t];			//换成相应的汉字数组得到点阵数据 
		else if(size==16)temp=hz16[index][t];     
		else if(size==24)temp=hz24[index][t];			
...
//	u16 i,j; 
	...
	// for (i = 0; i < ver; i++)
	// {
		LCD_WR_DATA16_N(color, ver*lenght);
	// }
}
...
void LCD_Show_array(u16 x,u16 y,u16 length,u16 width,u16 fc,u16 bc,const u8 pic[])
{
	u8 temp,sizex,t,m=0;
	u16 i,TypefaceNum;//一个字符所占字节大小
//	u16 x0=x;
	sizex = length;
	TypefaceNum=length * width / 8;
	LCD_Address_Set(x,y,x+length-1,y+width-1);  //设置光标位置
	for (i = 0; i < TypefaceNum; i++)
	{
		temp = pic[i]; // 调用16*16字体s
		for (t = 0; t < 8; t++)
		{
			if (temp & (0x01 << t))
				LCD_WR_DATA(fc);
